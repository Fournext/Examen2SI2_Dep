...
gestion = Gestion.objects.get(nombre="Gestion 2023")
niveles = Nivel.objects.all()
materias = Materia.objects.all()
trimestres = gestion.trimestres.all()

with open('datos_boletin.csv', mode='w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    writer.writerow([
        "id_estudiante", "nivel", "materia", "trimestre", 
        "ser", "saber", "hacer", "decidir", "promedio", "estado"
    ])

    for i in range(1000):
        user = Usuario.objects.create(
            username=f"estu2023_{i}",
            email=f"estu2023_{i}@mail.com",
            password="123456",
